...
@pytest.mark.parametrize('state, log_str, threshould, desired',[
    ...
    (InterfaceState(Status.RUNNING, FailState(None, datetime(2021, 12, 31), 2)),
     ...
     InterfaceState(Status.FAILURE, FailState(None, datetime(2021, 12, 31), 3))),
    ...
    # revive interface
    (InterfaceState(Status.FAILURE, FailState(None, datetime(2021, 12, 31), 2)),
     '20220101000000,1.1.1.1/30,1', 3,
     InterfaceState(Status.RUNNING, FailState((datetime(2021, 12, 31), datetime(2022, 1, 1)), None, 0))),
    # failure continued
    (InterfaceState(Status.FAILURE, FailState(None, datetime(2021, 12, 31), 3)),
     '20220101000000,1.1.1.1/30,-', 3,
     InterfaceState(Status.FAILURE, FailState(None, datetime(2021, 12, 31), 4))),
])
